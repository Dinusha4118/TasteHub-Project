...
function MyLearningPlan() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await axios.get('http://localhost:8086/learningPlan');
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`http://localhost:8086/learningPlan/${id}`);
      ...
      case 1:
        return (
          <div className="template_dis template-1">
            ...
            {post.imageUrl && (
              <img
                src={`http://localhost:8086/learningPlan/planImages/${post.imageUrl}`}
                alt={post.title}
