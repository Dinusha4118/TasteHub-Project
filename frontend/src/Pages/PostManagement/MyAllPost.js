...
function MyAllPost() {
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      try {
        const response = await axios.get('http://localhost:8086/posts');
        ...
        const ownerPromises = userIDs.map((userID) =>
          axios.get(`http://localhost:8086/user/${userID}`)
            .then((res) => ({
              userID,
  ...
  useEffect(() => {
    const fetchFollowedUsers = async () => {
      ...
      if (userID) {
        try {
          const response = await axios.get(`http://localhost:8086/user/${userID}/followedUsers`);
  ...
  const handleFollowToggle = async (postOwnerID) => {
    ...
    try {
      if (followedUsers.includes(postOwnerID)) {
        // Unfollow logic
        await axios.put(`http://localhost:8086/user/${userID}/unfollow`, { unfollowUserID: postOwnerID });
        setFollowedUsers(followedUsers.filter((id) => id !== postOwnerID));
      } else {
        // Follow logic
        await axios.put(`http://localhost:8086/user/${userID}/follow`, { followUserID: postOwnerID });
  ...
  const handleDeleteComment = async (postId, commentId) => {
    const userID = localStorage.getItem('userID');
    try {
      await axios.delete(`http://localhost:8086/posts/${postId}/comment/${commentId}`, {
        params: { userID },
      });
  ...
  const handleSaveComment = async (postId, commentId, content) => {
    try {
      const userID = localStorage.getItem('userID');
      await axios.put(`http://localhost:8086/posts/${postId}/comment/${commentId}`, {
        userID,
        content,
      });
                ...
                <div key={post.id} className='post_card'>
                  ...
                  <div className='user_details_card_di'>
                     <p className='card_post_title'>
                      <Link to={`/detailedPost/${post.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                          {post.title}
                      </Link>
                    </p>
                    ...
                    {post.media.slice(0, 4).map((mediaUrl, index) => (
                      ...
                      >
                        {mediaUrl.endsWith('.mp4') ? (
                          <video controls>
                            <source src={`http://localhost:8086${mediaUrl}`} type="video/mp4" />
                            Your browser does not support the video tag.
                          </video>
                        ) : (
                          <img src={`http://localhost:8086${mediaUrl}`} alt="Post Media" />
                        )}
      ...
      >
        ...
        {selectedMedia && selectedMedia.endsWith('.mp4') ? (
          <video controls className="modal-media">
            <source src={`http://localhost:8086${selectedMedia}`} type="video/mp4" />
            Your browser does not support the video tag.
          </video>
        ) : (
          <img src={`http://localhost:8086${selectedMedia}`} alt="Full Media" className="modal-media" />
        )}
      </Modal>
    </div >
  );
